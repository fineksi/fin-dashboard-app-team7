...
exports.findByEmail = async (email) => {
    const [rows] = await Db.query(`select * from ${TABLE_NAME} where email = ? and deleted_at is null limit 1`, [email]);
    return rows[0];
};

exports.updateLastLogin = async (email) => {
    const [status] = await Db.query(`UPDATE ${TABLE_NAME} SET last_login_at = NOW() WHERE email = ?`, [email]);
    return status;
}
