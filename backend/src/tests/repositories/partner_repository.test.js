...
test("findById should return a user if found", async(t) => {
    const mockUser = { uuid: "5b1ec29e-9c11-416a-b396-a527732f8718", id: 1, email: "sigma@example.com", password: "exampleHashed" };
    ...
    t.true(DbStub.query.calledWith("select * from partner where id = ? and deleted_at is null limit 1", [1]));
});
...
test("findById should return undefined if user doesn't exist", async(t) => {
    ...
    t.true(DbStub.query.calledWith("select * from partner where id = ? and deleted_at is null limit 1", [2]));
});
...
test("findByUuid should return a user if found", async(t) => {
    ...
    const user = await AuthRepository.findByUuid("5b1ec29e-9c11-416a-b396-a527732f8718");

    t.deepEqual(user, mockUser);
    t.true(DbStub.query.calledWith("select * from partner where uuid = ? and deleted_at is null limit 1", ["5b1ec29e-9c11-416a-b396-a527732f8718"]));
});

test("findByUUID should return undefined if user doesn't exist", async(t) => {
    DbStub.query.resolves([[]]);

    const user = await AuthRepository.findByUuid("55555555-ffff-cccc-6666-aaaaaaaaaaaaaa");

    t.deepEqual(user, undefined);
    t.true(DbStub.query.calledWith("select * from partner where uuid = ? and deleted_at is null limit 1", ["55555555-ffff-cccc-6666-aaaaaaaaaaaaaa"]));
});
...
test("findByEmail should return a user if found", async(t) => {
    ...
    t.deepEqual(user, mockUser);
    t.true(DbStub.query.calledWith("select * from partner where email = ? and deleted_at is null limit 1", ["sigma@example.com"]));
});
